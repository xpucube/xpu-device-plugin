...
func isGPUMemoryAssumedPod(pod *v1.Pod) (assumed bool) {
	log.V(6).Infof("Determine if the pod %v is XPU pre-allocated pod", pod)
	var ok bool

	// 1. check if it's for XPU share
	if getRequestXPUSharesFromPodResource(pod) <= 0 {
		log.V(6).Infof("Pod %s in namespace %s has not GPU Memory Request, so it's not XPU pre-allocated pod.",
			pod.Name,
			pod.Namespace)
		return assumed
	}
	...
	if _, ok = pod.ObjectMeta.Annotations[EnvResourceAssumeTime]; !ok {
		log.V(4).Infof("No assume timestamp for pod %s in namespace %s, so it's not XPU pre-allocated pod.",
			pod.Name,
			pod.Namespace)
		return assumed
	}
	...
	if assigned, ok := pod.ObjectMeta.Annotations[EnvAssignedFlag]; ok {
		...
		if assigned == "false" {
			log.V(4).Infof("Found XPU pre-allocated pod %s in namespace %s.",
				pod.Name,
				pod.Namespace)
			assumed = true
		} else {
			log.Infof("GPU assigned Flag for pod %s exists in namespace %s and its assigned status is %s, so it's not XPU pre-allocated pod.",
				pod.Name,
				pod.Namespace,
				assigned)
		}
	} else {
		log.Warningf("No GPU assigned Flag for pod %s in namespace %s, so it's not XPU pre-allocated pod.",
			pod.Name,
			pod.Namespace)
	}
...
func getRequestXPUSharesFromPodResource(pod *v1.Pod) uint {
	var total uint

...
func NewNvidiaDevicePlugin(mps, healthCheck bool) (*NvidiaDevicePlugin, error) {
	...
	log.Infof("nvidia devices map: %v", devNameMap)
	log.Infof("nvidia devices list: %v", devList)

	err := patchGPUCount(len(devList))
	//err := patchGPUCount(int(getXPUShares()))
	if err != nil {
		return nil, err
	}
	disableXPU, err := disableXPUOrNot()
	if err != nil {
		return nil, err
	}

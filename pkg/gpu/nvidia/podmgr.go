...
func disableXPUOrNot() (bool, error) {
    disable := false
...
func patchGPUCount(gpuCount int) error {
	...
	//newNode.Status.Capacity[resourceCount] = *resource.NewQuantity(int64(getXPUShares()), resource.DecimalSI)
	//newNode.Status.Allocatable[resourceCount] = *resource.NewQuantity(int64(getXPUShares()), resource.DecimalSI)
